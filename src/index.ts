...
import { confirmNotes, createDatabase, storageNotes } from './storageNotes';
...
ipcMain.handle("storage-notes", async (event, notes: string) => {
  try{
    const newNotes = (JSON.parse(notes)as INote[]).map(inote => Note.fromINote(inote))
    console.log(typeof newNotes[0].date)
    await storageNotes(newNotes)
...
ipcMain.handle("confirm-notes", async (event, notes)=>{
  try{
    await confirmNotes(notes)
  }
  catch(err){
